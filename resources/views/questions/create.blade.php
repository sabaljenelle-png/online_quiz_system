        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow p-6 mb-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h1 class="text-3xl font-bold text-slate-900">Add Question</h1>
                        <p class="text-slate-600 mt-1">Quiz: <span class="font-semibold">{{ $quiz->title }}</span></p>
                    </div>
                    <a href="{{ route('quizzes.show', $quiz) }}" class="text-indigo-600 hover:text-indigo-800 font-semibold">← Back to Quiz</a>
                </div>
            </div>
            ...
            @php
                $answerMode = old('answer_mode', 'radio');
                $oldOptions = old('options', ['', '', '', '']);
                while (count($oldOptions) < 4) { $oldOptions[] = ''; }
                $correctOptions = collect(old('correct_options', []))->map(fn($v) => (string) $v)->all();
            @endphp

            <form method="POST" action="/quizzes/{{ $quiz->id }}/questions" class="bg-white rounded-2xl shadow p-6 space-y-6">
                @csrf
                <input type="hidden" name="quiz_id" value="{{ $quiz->id }}">

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Question Text</label>
                    <textarea class="w-full border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" name="question_text" rows="4" required placeholder="Example: What command starts the Laravel local server?">{{ old('question_text') }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input class="w-full border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" type="number" name="points" value="{{ old('points', 1) }}" min="1" max="100" required>
                    </div>
                </div>
                ...
                <div id="multiple_choice_box" class="space-y-4">
                    ...
                    <div id="options_container" class="space-y-3">
                        @foreach ($oldOptions as $i => $optionText)
                            <div class="option-row flex flex-col md:flex-row md:items-center gap-3 p-3 rounded-xl border border-slate-200 bg-slate-50">
                                <input type="radio" name="correct_option" value="{{ $i }}" class="correct-radio text-indigo-600 focus:ring-indigo-500" {{ (string) old('correct_option', '0') === (string) $i ? 'checked' : '' }}>
                                ...
                                <input type="text" name="options[]" value="{{ $optionText }}" class="option-input flex-1 border-slate-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="Answer choice {{ $i + 1 }}{{ $i < 2 ? ' (required)' : ' (optional)' }}" {{ $i < 2 ? 'required' : '' }}>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 pt-2">
                        <p class="text-sm text-slate-500">Add another answer choice if this question needs more choices.</p>
                ...
                <div id="true_false_box" class="hidden space-y-3">
                    ...
                    <label class="flex items-center gap-3 border border-slate-200 rounded-lg p-3">
                        <input type="radio" name="true_false_answer" value="true" class="text-indigo-600 focus:ring-indigo-500" {{ old('true_false_answer', 'true') === 'true' ? 'checked' : '' }}>
                        <span>Correct answer is True</span>
                    </label>
                    <label class="flex items-center gap-3 border border-slate-200 rounded-lg p-3">
                        <input type="radio" name="true_false_answer" value="false" class="text-indigo-600 focus:ring-indigo-500" {{ old('true_false_answer') === 'false' ? 'checked' : '' }}>
                        <span>Correct answer is False</span>
                    </label>
                </div>

                <div class="flex justify-end gap-3 pt-2">
                    <button class="bg-green-600 text-white px-5 py-2 rounded-lg hover:bg-green-700" type="submit">Save Question</button>
                </div>
            </form>
        </div>
    </div>
